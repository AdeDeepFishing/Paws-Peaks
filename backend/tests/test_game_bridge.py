...
class GameBridgeTests(unittest.TestCase):
    ...
    def test_live_delegates_and_filters_status_fields(self):
        def pipeline(argv, *, output_folder, on_event):
            self.assertEqual(argv, ['--image', str(self.folder / 'input.png'), '--game-stage', 'river'])
        ...
        self.assertNotIn('secret', (self.folder / 'results.jsonl').read_text())
    ...
    def test_worker_listens_before_requests_and_reuses_process(self):
        ...
        for number in range(2):
            ...
            job.mkdir()
            shutil.copyfile(self.folder / 'input.png', job / 'input.png')
            run.save_job(job / 'request.json', {'request_id': 'test-' + str(number),
    ...
    def test_result_history_retains_image_paths_and_filters_private_fields(self):
        ...
        records = [json.loads(line) for line in (self.folder / 'results.jsonl').read_text().splitlines()]
    ...
    def test_pool_exception_is_logged_without_private_exception_text(self):
        ...
        self.assertNotIn('private-provider-token', (self.folder / 'results.jsonl').read_text())
