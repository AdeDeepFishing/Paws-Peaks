...
from common import ROOT, AppError, image_input
...
from sketch_to_narrative.run import validate_interpretation
...
def write_status(job_dir, status):
    ...
    with (job_dir / "results.jsonl").open("a", encoding="utf-8") as log:
...
def generate(job_dir, mode, emit, game_stage):
    ...
    with open(os.devnull, "w") as sink, contextlib.redirect_stdout(sink), contextlib.redirect_stderr(sink):
        code = pipeline.main(["--image", str(job_dir / "input.png"), "--game-stage", game_stage],
                             output_folder=job_dir / "artifacts", on_event=emit)
    return code



def execute(job_dir, request_id, encounter_id, mode, game_stage="river"):
...
def serve(worker_dir):
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        while not (worker_dir / "shutdown").exists():
            for request_path in sorted(worker_dir.glob("*/request.json")):
                ...
                try:
                    ...
                    future = executor.submit(execute, claimed.parent, request_id, value.get("encounter_id"),
                                             value["mode"], value["game_stage"])
                    identity = {"schema_version": 1, **{k: value.get(k) for k in
                                ("request_id", "encounter_id", "game_stage", "mode")}}
                    pending[future] = (claimed.parent, identity)
                except (OSError, ValueError, KeyError, TypeError):
                    write_status(claimed.parent, {
