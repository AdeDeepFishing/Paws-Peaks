...
func _ready() -> void:
	...
	for mesh in find_children("*", "MeshInstance3D", true, false):
		...
		for surface in mesh.mesh.get_surface_count():
			...
			if extras.has("windStrength"):
			...
			elif label == "Sky_dome" or label == "Painted_moon" or label.begins_with("Cloud_layer"):
				...
				mesh.set_surface_override_material(surface, materials[original])
		if label == "Continuous_organic_forest_terrain" or label.begins_with("Painted_rounded_rock") or label.begins_with("Flowing_painted_trunk") or label.begins_with("Buttress_root") or label.begins_with("Embedded_trail_edge_stone"):
