...
func run():
	...
	check(is_equal_approx(level.player.velocity.length(), level.player.sprint_speed), "Shift speed remains independent of the animation timer")
	...
	Input.action_press("move_up")
	await frames(10)
	check(level.player.visual.state == "walk", "Restarting while Shift is held still starts with walking")
	Input.action_release("move_up")
