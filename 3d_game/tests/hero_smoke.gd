...
func run():
	...
	Input.action_press("move_up")
	await frames(10)
	check(level.player.visual.state == "run", "Shift selects the fast movement delivery")
	...
	check(hero.animator.current_animation == "gameplay/idle", "Standing selects the static idle delivery")
	check(hero.standing_model.visible, "Default stand uses the delivered static mesh")
	var idle_time: float = hero.animator.current_animation_position
	await frames(10)
	check(hero.animator.current_animation_position > idle_time, "Idle playback remains active while standing")
	...
	Input.action_press("sprint")
	await frames(15)
	check(hero.state == "run", "Shift selects the running animation")
	Input.action_release("sprint")
	Input.action_release("move_up")
	await frames(5)
