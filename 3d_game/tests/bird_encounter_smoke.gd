...
func run():
	...
	check(level.bird.get_child_count() == 2 and level.bird.perch.visible != level.bird.visual.visible, "Exactly one giant bird guards the hill")
	check(level.bird.visual.get_child(0).scale.x == 4.0, "Bird is four times the delivered model size")
	check(level.bird.animators.size() == 2, "One bird has both flapping and breathing clips")
