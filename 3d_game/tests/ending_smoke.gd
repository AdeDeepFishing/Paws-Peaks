...
func run() -> void:
	...
	check(art.find_children("*", "MeshInstance3D", true, false).size() == 515, "All delivered dawn meshes are present")
	check(art.find_children("Cloud_layer*", "MeshInstance3D", true, false).size() == 20, "Dawn retains the layered cloud sky")
	check(not art.find_child("Painted_moon", true, false).visible and art.find_children("Luminous_body*", "MeshInstance3D", true, false).all(func(mesh): return not mesh.visible), "Dawn removes the moon and fireflies")
	check(art.find_child("DaybreakSunlight", true, false).shadow_enabled, "Sunrise casts terrain and character shadows")
	...
	check(animator.is_playing() and "Forest_wind_clouds_fireflies" in animator.current_animation, "Dawn wind and clouds animate")
	...
	check(ending.ambience.tonemap_exposure < 1.0 and ending.ambience.glow_bloom < 0.03, "Dawn stays softer than the former full-exposure ending")
